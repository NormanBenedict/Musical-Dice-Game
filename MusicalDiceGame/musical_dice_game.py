...
def construct_waltz():
    ...
    minuet_waltz = []
    measure_num = 0
    while measure_num < 16:
        x = minuet_table[measure_num]
        x = x[roll_dice(2)]
        x = sa.WaveObject.from_wave_file(minuet_filename(x))
        print(x)
        minuet_waltz.append(x)
        measure_num = measure_num +1
    measure_num = 0
    while measure_num < 16:
        y = trio_table[measure_num]
        y = y[roll_dice(1)]
        y = sa.WaveObject.from_wave_file(trio_filename(y))
        trio_waltz.append(y)
        measure_num = measure_num +1
    for i in minuet_waltz:
        player = i.play()
        player.wait_done()
    for x in trio_waltz:
        player = x.play()
